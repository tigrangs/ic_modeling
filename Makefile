...
changeToClang : 
	$(eval COMPILER := clang)
	$(eval LIBS += -lm -lstdc++)
	$(eval COMPILERFLAGS += -O1 -g -fsanitize=address -fno-omit-frame-pointer -g)
	$(eval LINKERFLAGS += -fsanitize=leak -g)
	@echo -e "$(GREEN)Using clang.$(RCOLOR)"
clang:	changeToClang install
_makeODir:
	@mkdir -p $(OBJ_DIR)
resolve:
...
$(execable): $(OBJS)
	@echo -e "$(BOLD)$(GREEN)$(COMPILER) $(COMPILERFLAGS) $(DEF_FLAGS) <obj_files> -o $@ $(LIBS) $(ENGINE_LIBS)$(RCOLOR)"
	@$(COMPILER) $(INCLUDES) $(COMPILERFLAGS) $(LINKERFLAGS) $(DEF_FLAGS) $(OBJS) -o $@ $(LIBS) $(ENGINE_LIBS)
clean:
	@rm -rf $(OBJ_DIR)
	@rm -rf $(execable)
...
help info:
	...
	@echo -e "$(YELLOW)ENGINE_LIBS$(RCOLOR)\n\tto set SDL libs."
	@echo -e "\tby default: '$(MAGENTA)$(ENGINE_LIBS)$(RCOLOR)'"
